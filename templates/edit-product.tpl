...
<div class="row justify-content-md-center">
    <form action="editProduct/{$product[0]->id}" method="POST" class="my-5">
        <div class="row">
            <div class="col-9">
                <div class="form-group">
                    <label>Producto</label>
                    <input name="name" type="text" class="form-control" value="{$product[0]->name}" required>
                </div>
            </div>
            <div class="col-3">
                <div class="form-group">
                    ...
                    <select name="id-brand" class="form-select">
                        ...
                        {foreach $brands as $brand}
                            <option {if $brand->id == $product[0]->id_brand}
                                selected
                            {/if} value="{$product[0]->id_brand}">{$brand->name}</option>
                        {/foreach}
                    </select>
            </div>
            </div>
        ...
        <div class="form-group">
            ...
            <input name="description" class="form-control" rows="3" maxlength="200" required value="{$product[0]->description}"></input>
        </div>
